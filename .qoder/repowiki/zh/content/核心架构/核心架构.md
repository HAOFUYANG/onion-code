...
## 简介
本架构文档面向 Onion Code 的核心系统，围绕“基于 LangChain 的智能体（Agent）”展开，系统采用 LangGraph 图式执行模型与 LangChain 工具体系，结合 SQLite 作为会话检查点存储，并通过受控的 Python 环境管理策略实现安全、可扩展的代码执行能力。本文重点阐述：
...
章节来源
- [agent.ts:20-142](file://src/agent/agent.ts#L20-L142)
- [tools.ts:1-10](file://src/agent/tools.ts#L1-L10)
...
## 架构总览
系统采用“LangGraph 图式执行 + LangChain 工具 + SQLite 检查点”的组合架构。Agent 通过系统提示词与工具集合进行推理与行动；会话历史通过 SQLite 检查点自动续接，实现多轮对话与状态恢复；Python 环境管理确保代码执行的安全与可重复。
...
图表来源
- [agent.ts:102-142](file://src/agent/agent.ts#L102-L142)
...
图表来源
- [cli.ts:79-186](file://src/agent/cli.ts#L79-L186)
...
图表来源
- [package.json:21-51](file://package.json#L21-L51)
...
## 结论
Onion Code 以 LangGraph 为核心，结合 LangChain 工具与 SQLite 检查点，构建了具备多轮会话、动态技能注入与可控代码执行能力的智能体系统。通过严格的 Python 环境管理与安全扫描策略，在保证安全性的同时兼顾易用性与可扩展性。未来可在以下方向持续演进：

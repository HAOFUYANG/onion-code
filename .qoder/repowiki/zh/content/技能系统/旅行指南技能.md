...
- [3-day-tokyo-itinerary.md](file://3-day-tokyo-itinerary.md)
...
## 简介
本指南面向“旅行指南”技能模块，系统讲解旅行规划能力的使用方法与工作原理，覆盖地点推荐、路线优化与个性化配置，并结合实际案例（东京三日游）演示生成流程。同时，文档说明参数配置、自定义扩展与最佳实践，帮助用户高效产出高质量旅行计划。

## 项目结构
旅行指南技能位于 skills 目录下的独立子模块，通过统一的技能发现与加载机制接入主系统；示例行程以 Markdown 文件形式提供，便于直接复用与参考。
...
- 示例输出：提供东京三日游的完整行程模板，涵盖住宿区域、每日活动、美食与交通建议、实用贴士与可选加游项目，便于直接套用与二次创作。
...
### 东京三日游行程生成示例
- 输入要素：目的地为东京，计划三日游，包含现代与传统两条主线，涵盖主要景点、美食与交通建议。
- 输出结构：包含住宿区域推荐、每日活动时间线、美食清单、交通指南与实用贴士，并提供可选加游项目。
- 使用建议：可直接套用模板，按需替换具体时间、餐厅与交通方式；也可作为旅行指南技能的输入样例，指导模型生成更个性化的行程。

```mermaid
flowchart TD
A["输入：东京三日游"] --> B["确定区域与主题<br/>现代侧/传统侧"]
B --> C["规划每日活动时间线<br/>景点/交通/用餐/夜生活"]
C --> D["汇总美食与交通建议"]
D --> E["生成实用贴士与可选加游"]
E --> F["输出：完整行程模板"]
```
...
## 结论
旅行指南技能通过标准化的信息收集与结构化输出，为用户提供可落地的旅行计划。配合示例模板与工具层的加载机制，用户可快速生成并迭代高质量行程。建议在实际使用中结合个人偏好与实时信息，持续优化生成结果。
...
- 自定义与扩展
  ...
  - 参考旅行指南示例，将模板迁移至新技能，形成可复用的旅行计划生成器。
- 最佳实践与实用技巧

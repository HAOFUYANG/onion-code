...
**更新** 工具系统现已集成统一的样式化日志输出功能，提供更好的调试信息和用户体验。所有工具调用都会产生格式化的彩色日志，包含工具名称、调用状态和详细信息。
...
- [style.ts:1-49](file://src/agent/style.ts#L1-L49)
...
- 日志样式化系统
  - 统一的日志样式定义，提供彩色工具调用日志、状态消息和欢迎横幅，增强调试体验。
- 文件操作工具
...
## 架构总览
工具系统以"LangChain 工具"为核心抽象，所有工具均遵循统一的接口规范：输入参数由 Zod Schema 描述，返回值为字符串或结构化结果；同时在实现层叠加多层安全策略与资源限制，确保在可控范围内提供强大能力。新增的日志样式化系统为所有工具调用提供统一的彩色输出格式。

```mermaid
sequenceDiagram
participant U as "调用者"
participant T as "工具实例"
participant ST as "样式化日志"
...
**图表来源**
- [exec.ts:118](file://src/agent/tools/exec.ts#L118)
- [read_file.ts:24](file://src/agent/tools/read_file.ts#L24)
- [load_skill.ts:16](file://src/agent/tools/load_skill.ts#L16)
- [style.ts:16](file://src/agent/style.ts#L16)
...
### 日志样式化系统（style）
- 功能概述
  - 提供统一的彩色日志输出样式，包含工具调用日志、状态消息和欢迎横幅。
- 关键特性
  - 工具调用日志：显示工具名称、调用状态和详细信息，使用黄色徽章和彩色标签。
  - 行数日志：专门用于代码执行工具，显示代码行数统计。
  - 品牌标识：包含洋葱品牌标识和用户提示符。
  - 状态消息：统一的停止、再见和错误消息样式。
- 使用要点
  - 所有工具调用前调用相应的日志函数。
  - 支持详细信息和行数两种日志格式。
...
```mermaid
flowchart TD
Start(["日志调用"]) --> Type{"日志类型"}
Type --> |工具调用| Tool["toolLog(工具名, 详情)"]
Type --> |行数统计| Lines["toolLogLines(工具名, 行数)"]
Tool --> Format["格式化彩色输出"]
Lines --> Format
Format --> Console["console.log 输出"]
```

**图表来源**
- [style.ts:16](file://src/agent/style.ts#L16)
- [style.ts:26](file://src/agent/style.ts#L26)
...
```mermaid
flowchart TD
Start(["函数入口"]) --> Log["显示工具调用日志"]
Log --> Resolve["解析并规范化路径"]
...
- **新增** 统一日志输出：调用前显示彩色工具调用日志，包含执行的命令详情。
...
- 使用要点
  - 先验证技能存在性，再加载内容。
- **新增** 统一日志输出：调用前显示彩色工具调用日志，包含技能名称详情。
...
- **新增** 日志输出性能
  - 彩色日志输出使用轻量级样式库，对性能影响可忽略不计。
  - 日志格式化在工具调用前执行，不影响主要业务逻辑性能。
...
- 排查步骤
  ...
  - 对网络请求检查代理、域名与防火墙设置。
- **新增** 日志相关排查
  - 检查控制台输出的彩色日志，确认工具调用是否正常触发。
  - 验证日志格式是否符合预期样式（工具名称、调用状态、详细信息）。
  - 确认日志输出是否包含必要的调试信息。
...
## 结论
工具系统通过统一的接口规范与多层安全策略，在保障安全性的同时提供了强大的文件操作、网络访问与代码执行能力。**新增的统一日志样式化系统**进一步提升了调试体验，所有工具调用都产生格式化的彩色输出，包含工具名称、调用状态和详细信息。通过共享的安全模块与一致的错误处理，开发者可以快速扩展新的工具，并在现有框架内获得稳定的运行体验。
...
- 创建新工具
  ...
  - **新增** 导入并使用日志样式化函数，在工具调用前添加适当的日志输出。
- 参数验证
...
- **新增** 日志样式化最佳实践
  - 使用 `toolLog()` 函数记录工具调用，包含必要的详细信息。
  - 对代码执行工具使用 `toolLogLines()` 函数，显示代码行数统计。
  - 保持日志格式一致性，便于调试和监控。
- 最佳实践
...
- 读取文件
  ...
  - **新增** 日志输出示例：[read_file.ts:24](file://src/agent/tools/read_file.ts#L24)
- 执行命令
  - 示例调用路径参考：[exec.test.ts:7-21](file://src/agent/tools/exec.test.ts#L7-L21)
  - **新增** 日志输出示例：[exec.ts:118](file://src/agent/tools/exec.ts#L118)
- 抓取网页
...
- 加载技能
  ...
  - **新增** 日志输出示例：[load_skill.ts:16](file://src/agent/tools/load_skill.ts#L16)